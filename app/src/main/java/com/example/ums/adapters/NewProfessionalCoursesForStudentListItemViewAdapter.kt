...
class NewProfessionalCoursesForStudentListItemViewAdapter(private val studentID: Int, private val courseDAO: CourseDAO, private val studentDAO: StudentDAO, private val clickListener: ClickListener) : RecyclerView.Adapter<ClickableListItemViewHolder>() {

    private var originalList : MutableList<Course> = courseDAO.getNewProfessionalCourses(studentID).sortedBy { it.id }.toMutableList()
    ...
    fun filter(query: String?){
        val filteredList =
            if(query.isNullOrEmpty())
                courseDAO.getNewProfessionalCourses(studentID).sortedBy { it.id }.toMutableList()
            else
                courseDAO.getNewProfessionalCourses(studentID).filter { course -> course.name.contains(query, true) }.sortedBy { course ->  course.id }
